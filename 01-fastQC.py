...
def check_arg (args=None) :
    ...
    parser = argparse.ArgumentParser(prog = '01-fastQC.py', formatter_class=argparse.RawDescriptionHelpFormatter, description= '01-fastQC.py creates a csv file from fastqc.txt file')

    parser.add_argument('--path','-p', required=True, help='Insert path of fatsqc.txt file from 01-fastQC folder like /home/user/Service_folder/ANALYSIS/01-fastQC') 
    ...
    with open (file_txt, 'r') as fd:
        for line in fd:
            ...
            if header:
                line = line.replace('\n','') 
                line = line.split('\t')
                qc_dict[step + '_pre_' + line[0]] = line[1]
            if m:
                header = True
    ...
    for sample in sample_list:
        ...
        for step in steps:
            ...
            if os.path.exists(file_name)== False:
                ...
                if not fastq_all_pre[sample]:
                    fastq_all_pre[sample] = dic_tmp
                else:
                    #fastq_all_pre[sample] = {**fastq_all_pre[sample],**dic_tmp} 
                    fastq_all_pre[sample].update(dic_tmp)
    
    print ('fastq_all_pre done')
    print (fastq_all_pre)

    # Save the dicctionary to binary file
    
    fastqc_dictionary_bn (fastq_all_pre, arguments.output_bn)
        
    print ('fastq_dictionary_bn done')
    
    # Convert the dictionary to csv file
    
    fastqc_dictionary_csv (fastq_all_pre, arguments.output_csv, sample_list)
